RSpec.describe Locust::Keywords::WriteOnly::Validator, ".call" do
  subject { described_class.call(schema) }

  let(:grandpa) { double keyword: "properties" }
  let(:parent)  { double :object, read_only: nil, parent: grandpa }
  let(:source)  { false }
  let(:schema) { double parent: parent, source: source, full_path: "writeOnly" }
  ...
  context "when described object is read- and write-only at the same time" do
    let(:parent) { double :object, read_only: read, parent: grandpa }
    let(:source) { true }
    let(:read)   { double source: true }
